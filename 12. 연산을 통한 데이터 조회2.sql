...
5. 정규식(regex)을 이용한 패턴 연산
   ...
   http://zvon.org/comp/r/tut-Regexp.html#Pages~Contents
    ...
    [Quantifiers]
    ?	없거나 있거나 (zero or one) //gra?y = 위치에 a가 1개있거나 없거나 한 단어들(ex)gray, gry)
    *	없거나 있거나 많거나 (zero or more) //gra?y = 위치에 a가 1개있거나 없거나 많거나 한 단어들(ex)gray, gry, graay..)
    +	하나 또는 많이 (one or more) //gra?y = 위치에 a가 1개이상 있는 단어들(ex)gray, graay)   
    {n}	n번 반복
    {min,}	최소
    {min,max}	최소, 그리고 최대
